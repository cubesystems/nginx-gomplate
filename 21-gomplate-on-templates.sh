...
auto_envsubst() {
  ...
  local suffix="${NGINX_ENVSUBST_GOMPLATE_SUFFIX:-.gomplate}"
  local output_dir="${NGINX_ENVSUBST_OUTPUT_DIR:-/etc/nginx/conf.d}"
  local stream_suffix="${NGINX_ENVSUBST_STREAM_GOMPLATE_SUFFIX:-.stream-gomplate}"
  ...
  find "$template_dir" -follow -type f -name "*$suffix" -print | while read -r template; do
    ...
    entrypoint_log "$ME: Running gomplate on $template to $output_path"
    gomplate -f "$template" -o "$output_path"
  done
